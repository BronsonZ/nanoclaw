...
function findWorkspace(
  ...
  if (config) {
    const agentWs = (config.agent as Record<string, unknown> | undefined)
      ?.workspace as string | undefined;
    if (agentWs)
      configPaths.push(
        agentWs.startsWith('~')
          ? path.join(os.homedir(), agentWs.slice(1))
          : agentWs,
      );
    const defaultsWs = (
    ...
    if (defaultsWs)
      configPaths.push(
        defaultsWs.startsWith('~')
          ? path.join(os.homedir(), defaultsWs.slice(1))
          : defaultsWs,
      );
  }
...
function main(): void {
  ...
  const { dir: workspaceDir, files: workspaceFiles } = findWorkspace(
    stateDir,
    config,
  );
