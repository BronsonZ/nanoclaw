...
/** Telegram expires typing status after ~5s; re-send before that. */
...
export class TelegramChannel implements Channel {
  ...
  async sendMessage(
    ...
    // Telegram cancels typing when a message is sent — clear our refresh interval
    // to match, preventing the interval from re-enabling typing after the response.
  ...
  async setTyping(jid: string, isTyping: boolean): Promise<void> {
    // Always clear any existing interval for this jid first
    ...
    // Send immediately, then refresh on an interval
    send();
    this.typingIntervals.set(jid, setInterval(send, TYPING_REFRESH_MS));
  }
}
